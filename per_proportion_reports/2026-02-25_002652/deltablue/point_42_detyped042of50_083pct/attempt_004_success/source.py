...
def chain_test(_n):
    ...
    edits = []
    edits.append(edit)
    plan = planner.extract_plan_from_constraints(CheckedList[UrnaryConstraint](edits))
    i = box(int64(0))
    while int64(i) < 100:
        first.value = int64(i)
        plan.execute()
...
def projection_test(_n):
    ...
    i = box(int64(0))
    while int64(i) < n - 1:
        if cast(Variable, dests[int64(i)]).value != int64(i) * 5 + 1000:
            print('Projection 3 failed')
        i = box(int64(int64(i) + 1))
    change(cast(Variable, offset), box(int64(2000)))
    i = box(int64(0))
    while int64(i) < n - 1:
        if cast(Variable, dests[int64(i)]).value != int64(i) * 5 + 2000:
