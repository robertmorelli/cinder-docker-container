"""Pure AST transform pipeline for detyping.

Extracted from CinderDetyperBoxUnbox._detype_funs. This module contains
the complete transform logic with no file I/O, subprocess, or class state.
All required context is passed explicitly via parameters.
"""
...
from passes.cleanup_checkedlist_return_wraps import cleanup_checkedlist_return_wraps
from passes.body_checkedlist_flow import apply as apply_body_checkedlist_flow
...
from passes.bins import scalar_construct_types
...
# Import pass specs/names from de_typer_boxunbox to avoid re-deriving.
# These are needed for pass-state lookups within the transform.
...
PASS_SPECS: tuple[tuple[str, str, str, str], ...] = tuple(
    (scope_name, policy_name, bin_name, f"{scope_name}_{policy_name}_{bin_name}")
...
def detype_funs(
    ...
    *,
    ...
    format_source: callable,
) -> str:
    """Pure AST transform: apply detyping passes according to guide, return rendered source."""

    def strict_fail(limitation_name: str, detail: str):
        if not strict_limitations:
            return
    ...
    def get_fun_q_names(fun_name: str, antr_name=None):
        assert antr_name in class_antrs, "class ancestor graph incomplete"
        anters = class_antrs[antr_name]
        anter_fun_names = tuple(
            q_name for q_name in zip(anters, (fun_name,) * len(anters)) if q_name in fun_names
        )
        return ((antr_name, fun_name),) + anter_fun_names

    @cache
    def fun_q_names_by_method_name(fun_name: str) -> tuple[GuideKey, ...]:
        return tuple(q_fun_name for q_fun_name in fun_names if q_fun_name[1] == fun_name)
        ...
        def normalize_cast_annotation(annotation_expr: expr):
            ...
            if isinstance(annotation_expr, Name) and annotation_expr.id in imported_symbols:
                strict_fail(
                    ...
                    f"cast target uses imported name {annotation_expr.id} in {benchmark_file_name}",
                )
            if (
                ...
                strict_fail(
                    "imported_nominal_cast",
                    f"cast target uses imported module attribute {annotation_expr.value.id}.{annotation_expr.attr} in {benchmark_file_name}",
                )
    ...
    def is_construct_wrapper_call(node: expr, annotation: expr) -> bool:
        return (
    ...
    def function_should_remove_types(fun_name: str, class_name: str | None):
        if class_name is not None and class_name not in class_antrs:
            return False
        try:
    ...
    def consistent_method_return_annotations(table: dict[GuideKey, dict]):
        ...
        for q_fun_name, entry in table.items():
            antr_name, method_name = q_fun_name
            ...
            if ret_ann is None:
                blocked.add(method_name)
                out.pop(method_name, None)
                continue
            existing = out.get(method_name)
            if existing is None:
                out[method_name] = deepcopy(ret_ann)
                continue
    ...
    class BodyDetyper(NodeTransformer):
        ...
        def _embed_write(self, node: expr, annotation: expr | None):
            ...
            if policy == "construct":
                # For Checked* body annassign erasure, preserve value identity in body flow.
                return node
            if policy == "cast":
                return wrap_cast_or_construct(annotation, node)
            return node

        def _project_read(self, node: expr, annotation: expr | None):
            policy = annotation_policy(annotation)
            if policy == "box":
        ...
        def visit_Assign(self, node: Assign):
            ...
            if len(node.targets) == 1 and isinstance(node.targets[0], Name):
                ...
                if target_name in self.local_reproject_annotations:
                    ...
                    node.value = self._embed_write(value, annotation)
        ...
        def visit_Name(self, node: Name):
            if isinstance(node.ctx, Load) and node.id in self.local_reproject_annotations:
                return self._project_read(node, deepcopy(self.local_reproject_annotations[node.id]))
            return node
        ...
        def visit_Attribute(self, node: Attribute):
            ...
            if not self._field_pass_enabled(field_ann):
                return node
            return self._project_read(node, field_ann)
    ...
    class BoundaryCallRetyper(NodeTransformer):
        def __init__(
            ...
            method_arg_fallback,
            field_annotations,
            local_ambiguous_method_names,
            local_strict_limitations: bool,
        ):
            ...
            self.field_annotations = field_annotations
            self.ambiguous_method_names = local_ambiguous_method_names
            self.strict_limitations = local_strict_limitations
            self.class_stack: list[str] = []
        ...
        def _embed_boundary_arg(self, node: expr, annotation: expr | None):
            ...
            if policy == "construct":
                ...
                if is_construct_wrapper_call(node, annotation):
                    return node
    ...
    def detype_function(fn: FunctionDef, pass_state: PassState, class_name: str | None):
        ...
        apply_body_checkedlist_flow(fn, pass_state, pass_ctx)
    ...
    class_names = set(class_name for class_name in class_antrs.keys() if class_name is not None)
